...
const CompletedTodos = ({ tasks, fetchTodos }: TodoProps): JSX.Element => {
    ...
    <Cont>
      <Ulcontainer task={tasks} mode={mode}>
        {Completed.map((complete, index) => (
          <TaskDiv mode={mode} done={complete.completed} key={index}>
            <li className="todo">
              <div
                className="isDone"
                onClick={() => updateTodo(complete._id, complete.completed)}
              >
                <img src={check} alt="" />
              </div>
              ...
              <svg
                ...
                onClick={() => deleteTodo(complete._id)}
